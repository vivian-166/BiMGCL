...
sys.path.append("") # add workspace path
import Process
...
def classify(treeDic, x_test  , x_train,TDdroprate,BUdroprate,lr, weight_decay,patience,n_epochs,batchsize,dataname,iter, fold_count):
    ...
    for unsup_epoch in range(25):
        ...
        train_loader = DataLoader(traindata_list, batch_size=batchsize['unsup_batchsize'], shuffle=True, num_workers=4)
        batch_idx = 0
        loss_all = 0
        ...
        for Batch_data in tqdm_train_loader:
            ...
            loss = unsup_model(Batch_data, Batch_data.rootindex)
            loss_all += loss.item() * (max(Batch_data.batch) + 1)
            print(f'unsup loss is:{loss.item()}')
            loss.backward()
            optimizer.step()
    ...
    model = Classfier(64*2*2,64,4,unsup_model.td_encoder, unsup_model.bu_encoder).to(device)
    ...
    for epoch in range(n_epochs):
        ...
        train_loader = DataLoader(traindata_list, batch_size=batchsize['sup_train_batchsize'], shuffle=True, num_workers=4)
        test_loader = DataLoader(testdata_list, batch_size=batchsize['sup_test_batchsize'], shuffle=True, num_workers=4)
        avg_loss = []
        avg_acc = []
        batch_idx = 0
        tqdm_train_loader = tqdm(train_loader)
        ...
        for Batch_data in tqdm_train_loader:
            ...
            out_labels= model(Batch_data.x, Batch_data.batch, Batch_data.rootindex, Batch_data.BU_edge_index_ori)
        ...
        for Batch_data in tqdm_test_loader:
            Batch_data.to(device)
            val_out = model(Batch_data.x, Batch_data.batch, Batch_data.rootindex, Batch_data.BU_edge_index_ori)
        ...
        early_stopping(np.mean(temp_val_losses), np.mean(temp_val_accs), np.mean(temp_val_F1), np.mean(temp_val_F2),
                       np.mean(temp_val_F3), np.mean(temp_val_F4), model, 'BiMGCL_'+str(fold_count)+'_', dataname)
...
weight_decay=1e-4
patience=20
batchsize={'unsup_batchsize':256, 'sup_train_batchsize':256, 'sup_test_batchsize':256}
TDdroprate=0.4
BUdroprate=0.4
